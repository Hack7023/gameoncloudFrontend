...
const CardMemory = () => {
  ...
  const [bestScore, setBestScore] = useState(
    sessionStorage.getItem("memorygame") == -1
      ? Number.POSITIVE_INFINITY
      : sessionStorage.getItem("memorygame") || Number.POSITIVE_INFINITY // for 1st time bestscore is infinity
  ); //to store best score in local storage
  // console.log(sessionStorage.getItem("memorygame"), "momo");
  ...
  const Handleclick = (index) => {
    // on clicking a card pass index
    if (openCards.length === 1) {
      // check the length of opencards i.e. 0 initially
  ...
  const handleRestart = () => {
    // for restarts sets all usestates to its initial state
    setMatchedcards({});
    setOpencards([]);
    setShowModal(false);
    setMoves(0);
